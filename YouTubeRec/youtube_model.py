...
import numpy as np
from data_util import config as cfg
...
class video_model(object):
    ...
    def youtube_network(self, history, example_age, num_classes, labels= None):
        item_embedding = tf.get_variable('age_embedding',[num_classes,24],\
                                         initializer=tf.variance_scaling_initializer())  # item * H
        history_vec = tf.nn.embedding_lookup(item_embedding,history) #  B * T * H
        print('history_vec',history_vec)
        inputs = tf.reduce_mean(history_vec,axis=1)                # B * H
        print('history_vec_mean',inputs)
        #ex_age = tf.square(example_age)
        inputs = tf.concat([inputs, example_age],axis=1)          # B * H+1
        print('inputs',inputs)
        
        net = tf.layers.dense(inputs,128,kernel_regularizer=self.kernel_regu,activation=tf.nn.relu,name='fc1') # B*128 
        ...
        net = tf.layers.batch_normalization(net,training = self.training,name = 'fc3_bn') # B*24
        ...
        logits = tf.matmul(net, tf.transpose(weights))  # B * item
