...
import book_code.{ItemPref, ItemSimi, ItemSimilarity, UserRecommd}
...
object I2iTest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 1.1读取item配置表
    val item_conf_path = "liuhongbing948/movies.csv"
    ...
    // 1.2读取用户行为数据
    val user_rating_path = "liuhongbing948/ratings.csv"
    ...
    val user_predictr_cooccurrence = ItemSimilarity.Recommend(cooccurrence, user_ds).map {
      case UserRecommd(userid: String, itemid: String, pref: Double) =>
