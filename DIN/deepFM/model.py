...
class Model(object):
    ...
    def __init__(self, user_count, item_count, cate_count, cate_list):
        ...
        user_emb_w = tf.get_variable("user_emb_w", [user_count, hidden_units])      # user_num * H
        item_emb_w = tf.get_variable("item_emb_w", [item_count, hidden_units // 2]) # item_num * H//2
        item_b = tf.get_variable("item_b", [item_count],initializer=tf.constant_initializer(0.0)) # item_num
        cate_emb_w = tf.get_variable("cate_emb_w", [cate_count, hidden_units // 2]) # cate_num * H//2
        cate_list = tf.convert_to_tensor(cate_list, dtype=tf.int64)

        u_emb = tf.nn.embedding_lookup(user_emb_w, self.u)

        ic = tf.gather(cate_list, self.i)     # B * H//2
