...
class Model(object):
    ...
    def __init__(self, user_count, item_count, cate_count, cate_list):
        ...
        userEmbedding 
        '''
     #   user_emb_w = tf.get_variable("user_emb_w", [user_count, hidden_units])      # 192403 * H
        item_emb_w = tf.get_variable("item_emb_w", [item_count, hidden_units // 2]) # 63001 * H//2
        item_b = tf.get_variable("item_b", [item_count],initializer=tf.constant_initializer(0.0)) # 63001
        cate_emb_w = tf.get_variable("cate_emb_w", [cate_count, hidden_units // 2])    # 801*H//2
        cate_list = tf.convert_to_tensor(cate_list, dtype=tf.int64)                  # 801
     ...
     #   u_emb = tf.nn.embedding_lookup(user_emb_w, self.u)  # B*H
        ...
        hc = tf.gather(cate_list, self.hist_i)               # B*T*H
        h_emb = tf.concat([
        ...
        tmp = tf.cast(tf.tile(tf.expand_dims(self.sl,1), [1,hidden_units]), tf.float32)  # B*H
        hist = tf.div(hist, tmp)     

        '''
        不等价与下面的，因为 tmp长度不一定相等，用户历史item个数不同
        hist = tf.reduce_mean(hist, 1)  # B*H
        ''' 
        ...
        d_layer_wide_i = tf.concat([tf.gather(u_emb, [0], axis=-1) * tf.gather(i_emb, [0], axis=-1), 
                                    tf.gather(u_emb, [hidden_units-1], axis=-1) * tf.gather(i_emb, [hidden_units-1], axis=-1),
                                    tf.gather(u_emb, [hidden_units // 2], axis=-1) * tf.gather(i_emb, [hidden_units // 2], axis=-1)], axis=-1)
        
        d_layer_wide_i = tf.layers.dense(d_layer_wide_i, 1, activation=None, name='f_wide') # B*1
        ...
        print("id:", i_b)
        print("d_layer_3_i:",d_layer_3_i)
        print("d_layer_wide_i:",d_layer_wide_i)
        ...
        u_emb_all = tf.tile(u_emb_all, [1, item_count, 1]) # B*63001*1
        # logits for all item:
        all_emb = tf.concat([
                item_emb_w,
        ...
        d_layer_wide_all = tf.concat([tf.gather(u_emb_all, [0], axis=-1) * tf.gather(all_emb, [0], axis=-1), tf.gather(u_emb_all, [hidden_units-1], axis=-1) * tf.gather(all_emb, [hidden_units-1], axis=-1), tf.gather(u_emb_all, [hidden_units // 2], axis=-1) * tf.gather(all_emb, [hidden_units // 2], axis=-1)], axis=-1)
        ...
        d_layer_wide_j = tf.concat([tf.gather(u_emb, [0], axis=-1) * tf.gather(j_emb, [0], axis=-1), 
                                    tf.gather(u_emb, [hidden_units-1], axis=-1) * tf.gather(j_emb, [hidden_units-1], axis=-1),
